...
# Load the fine-tuned "all-mpnet-base-v2" embedding model
embedding_model_mpnet = SentenceTransformer("all-mpnet-base-v2")

# Initialize BERTopic with the fine-tuned embedding model
topic_model_mpnet = BERTopic(embedding_model=embedding_model_mpnet, min_topic_size=35, verbose=True)

# Fit the model on the Reddit data
topics, _ = topic_model_mpnet.fit_transform(reddits)

# Display topic information
topic_info = topic_model_mpnet.get_topic_info()
print("Top 10 Topics:")
print(topic_info.head(10))

# Inspect a specific topic (e.g., topic 4)
print("\nTopic 4:")
print(topic_model_mpnet.get_topic(4))

# Inspect outlier topics (topic -1)
print("\nOutlier Topics:")
print(topic_model_mpnet.get_topic(-1))

# Visualize topics
topic_model_mpnet.visualize_topics()

# Visualize topics over time
topics_over_time = topic_model_mpnet.topics_over_time(reddits, timestamps, global_tuning=True, evolution_tuning=True, nr_bins=15)
topic_model_mpnet.visualize_topics_over_time(topics_over_time, top_n_topics=15)
...
coherence_cv, coherence_umass = calculate_coherence(topic_model_mpnet, tokenized_texts)

# Calculate Topic Diversity (TD) score
td_score = calculate_topic_diversity(topic_model_mpnet, top_n=10)
